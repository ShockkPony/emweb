...
exports.Server = function()
{
	...
	this.finish_config = function()
	{
		...
		try
		{
			var watch = fs.watch(this.config_file, function(e, name)
			{
				switch(e)
				{
					case 'change':
	...
	this.init = function()
	{
		...
		this.start_date_utc = this.start_date.toUTCString();
		...
		// setTimeout does not exist in node v0.8.x
		if(typeof this.server.setTimeout === 'function')
		{
			this.server.setTimeout(this.server_timeout);
		}
	...
	this.cb_request = function(request, response)
	{
		...
		if(this.do_log_user_agent) this.log('HTTP', 'user agent = ' + util.inspect(request.headers['user-agent']));
	...
	this.do_route = function(path, request, response, request_url, fallback_404)
	{
		...
		if(data === undefined || typeof this.handlers[route] !== 'function')
		{
			...
			if(fallback_404)
			{
				...
				response.setHeader('Last-Modified', this.start_date_utc);
		...
		if(modified)
		{
			...
			response.setHeader('Last-Modified', this.modified_cache[path]);
		}
	...
	{
		this.log('HTTP', 'continue request denied');
	...
	// set default config at end of constructor
	this.reset_config();
}
